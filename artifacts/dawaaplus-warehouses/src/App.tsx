    ...
    <div dir="rtl" style={{ display:"flex", height:"100vh", fontFamily:"'Segoe UI',Tahoma,Arial,sans-serif", overflow:"hidden" }}>
      ...
      <div style={{ flex:1, display:"flex", flexDirection:"column", overflow:"hidden" }}>
        ...
        <div style={{ flex:1, overflowY:"auto", padding:20, background:C.bg }}>
          ...
          {sec==="sub"    && <SubPage wh={wh} plan={wh.plan} color={C.primary} />}
...
function SubPage({ wh, plan, color }:any) {
  ...
  const curPlan = WH_PLANS.find(p=>p.id===plan)||WH_PLANS[0];
  const submitReq = () => {
    if (!selPlanId||!txRef.trim()) return;
    const req = { id:`REQ-${Date.now()}`, subscriberId:wh.id, subscriberType:"warehouse", subscriberName:wh.name, subscriberPhone:wh.phone, subscriberCity:wh.city, currentPlan:plan, requestedPlan:selPlanId, paymentMethod:payMethod, paymentLabel:acct.label, transactionRef:txRef.trim(), amount:selPlan?.price||0, date:new Date().toISOString().slice(0,10), status:"pending" };
  ...
  return (
    <div>
      <Card style={{ background:`linear-gradient(135deg,${color}12,${color}06)`,border:`2px solid ${color}35`,marginBottom:16 }}>
        <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center" }}>
          <div>
            ...
            <div style={{ fontSize:12,color:C.muted,marginTop:2 }}>{plan==="free"?"200 منتج · 3 صيدليات · بدون إعلانات":plan==="standard"?"2000 منتج · 20 صيدلية · إعلان/شهر":"غير محدود · 3 إعلانات · مدير حساب"}</div>
          </div>
      ...
      {pendingReq&&<div style={{ background:"#FFFDE7",border:"2px dashed #D69E2E",borderRadius:14,padding:"14px 16px",marginBottom:16,display:"flex",gap:12,alignItems:"center" }}>
        ...
        <div style={{ fontSize:12,color:C.muted }}>ترقية إلى {WH_PLANS.find(p=>p.id===pendingReq.requestedPlan)?.name} · المرجع: <b>{pendingReq.transactionRef}</b></div>
      ...
      {step==="plans"&&<div>
        ...
        {selPlanId&&selPlanId!==plan&&!pendingReq&&<button onClick={()=>setStep("pay")} style={{ background:`linear-gradient(135deg,${color},#0A6144)`,color:"#fff",border:"none",borderRadius:12,padding:"13px 28px",fontWeight:800,cursor:"pointer",fontSize:14 }}>متابعة للدفع →</button>}
      </div>}
      {step==="pay"&&selPlan&&<Card style={{ border:`2px solid ${color}40` }}>
        ...
        <div style={{ display:"flex",gap:10 }}>
          ...
          <button onClick={submitReq} disabled={!txRef.trim()} style={{ flex:1,background:txRef.trim()?`linear-gradient(135deg,${color},#0A6144)`:"#ccc",color:"#fff",border:"none",borderRadius:10,padding:11,fontWeight:800,cursor:txRef.trim()?"pointer":"not-allowed",fontSize:14 }}>📨 إرسال طلب الترقية</button>
        </div>
      </Card>}
  ...
  return (
    <div style={{ background:"#fff",borderBottom:`1px solid ${C.border}`,padding:"0 20px",height:56,display:"flex",alignItems:"center",gap:12,flexShrink:0 }}>
      ...
      <div style={{ display:"flex",alignItems:"center",gap:8 }}>
        <PlatformHub color={color} portalUrl={portalUrl} currentIcon="🏭" currentLabel="warehouse" />
