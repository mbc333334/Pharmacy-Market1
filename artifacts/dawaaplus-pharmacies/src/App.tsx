...
export default function App() {
  ...
  const [products, setProducts] = useState(()=>LS("ph_products",INIT_PRODUCTS));
  const [orders, setOrders] = useState(()=>LS("ph_orders",INIT_ORDERS));
  const [profile, setProfile] = useState<any>(null);

  useEffect(()=>{ if(ph) setProfile(LS(`ph_profile_${ph.id}`,ph)); },[ph]);
  useEffect(()=>{ localStorage.setItem("ph_products",JSON.stringify(products)); },[products]);
  useEffect(()=>{ localStorage.setItem("ph_orders",JSON.stringify(orders)); },[orders]);

  if (!ph) return <Login onLogin={p=>setPh(p)} />;
  return (
    <div dir="rtl" style={{ display:"flex", height:"100vh", fontFamily:"'Segoe UI',Tahoma,Arial,sans-serif", overflow:"hidden" }}>
      <Sidebar color={C.primary} icon="💊" title="بوابة الصيدلية" name={ph.name}
        menu={[{id:"dash",l:"لوحة التحكم",i:"📊"},{id:"acc",l:"حسابي",i:"👤"},{id:"inv",l:"المخزون",i:"📦"},
          {id:"orders",l:"الطلبات",i:"🛒"},{id:"sub",l:"الاشتراك",i:"💎"},{id:"fin",l:"المالية",i:"💰"},{id:"sup",l:"الدعم",i:"💬"}]}
        active={sec} onNav={setSec} onLogout={()=>setPh(null)} />
      <div style={{ flex:1, display:"flex", flexDirection:"column", overflow:"hidden" }}>
        <TopBar color={C.primary} label={[{id:"dash",l:"لوحة التحكم"},{id:"acc",l:"حسابي"},{id:"inv",l:"المخزون"},{id:"orders",l:"الطلبات"},{id:"sub",l:"الاشتراك"},{id:"fin",l:"المالية"},{id:"sup",l:"الدعم"}].find(m=>m.id===sec)?.l||""} name={ph.name} />
        <div style={{ flex:1, overflowY:"auto", padding:20, background:C.bg }}>
          {sec==="dash"   && <PhDash ph={ph} products={products} orders={orders} />}
          {sec==="acc"    && <PhAccount ph={profile||ph} onSave={(d:any)=>{ setProfile(d); localStorage.setItem(`ph_profile_${ph.id}`,JSON.stringify(d)); }} />}
